		...
		try {
			...
			//if (cartService.checkTotalAmountAgainstCart(total_amt, user_Id)) {
				...
				for (AddtoCart addCart : cartItems) {
					String orderId = "" + getOrderId(); //create a random order ID number using getOrderId method below
			...
			//} else {
			//	throw new Exception("There is a mismatch in total price and the JSON price");
			//}
		} catch (Exception e) {
			e.printStackTrace();
